...
module.exports = {
  ...
  changeController: (wallet, controller) => {
    return (newController, callback) => {
      if (utils.bytesVerify(newController, 20))
        createAndSendTx(wallet, controller, '0x3cebb823', padLeft(newController, 32), callback)
      else 
        callback(400, Messages.INVALID_INPUT)
    }
  },
  ...
  changeMaster: (wallet, controller) => {
    return (newMaster, callback) => {
      if (utils.bytesVerify(newMaster, 20))
        createAndSendTx(wallet, controller, '0xf4ff78bf', padLeft(newMaster, 32), callback)
      else 
        callback(400, Messages.INVALID_INPUT)
    }
  },
  ...
  setTarget: (wallet, controller) => {
    return (id, target, callback) => {
      if (utils.bytesVerify(id, 4) && utils.bytesVerify(target, 20))
        createAndSendTx(wallet, controller, '0x08af4b45', padRight(id, 32) + padLeft(target, 32), callback)
      else 
        callback(400, Messages.INVALID_INPUT)
    }
  },
  ...
  approve: (wallet, universal) => {
    return (approved, tokenId, callback) => {
      createAndSendTx(wallet, universal, '0x095ea7b3', padLeft(approved, 32) + padLeft(tokenId, 32), callback)
    }
  },

  createRecord: (wallet, universal) => {
    return (record, owner, participant, owner_signature, participant_signature, callback) => { 
  ...
  safeTransferFrom: (wallet, universal) => {
    return (from, to, tokenId, extra_data, callback) => { 
      ...
      if (extra_data && extra_data.length - 2) {
        ...
        selector = '0xb88d4fde'
        data = [from, to, tokenId, '0x80', extra_data.length / 2].map((val) => { return padLeft(val, 32) }).join('') 
        ...
        data = [from, to, tokenId].map((val) => { return padLeft(val, 32) }).join('')
        selector = '0x42842e0e'
      }
      if (
  ...
  setApprovalForAll: (wallet, universal) => {
    return (operator, approved, callback) => {
      ...
      if (
        ...
        createAndSendTx(wallet, universal, '0xa22cb465', padLeft(operator, 32) + padLeft(approved_boolean, 32), callback)
      else 
        callback(400, Messages.INVALID_INPUT)
    }
  },
  ...
  signedTransfer: (wallet, universal) =>{
    return (from, to, tokenId, signature, callback) => { 
      let data = [from, to, tokenId, '0x80', '0x41'].map((val) => { return padLeft(val, 32) }).join('')
  ...
  transferFrom: (wallet, universal) => {
    return (from, to, tokenId, callback) => { 
      let data = [from, to, tokenId].map((val) => { return padLeft(val, 32) })
      if (
        utils.bytesVerify(from, 20) &&
        utils.bytesVerify(to, 20) &&
        utils.bytesVerify(tokenId, 32) 
  ...
  versionRecord: (wallet, universal) => {
    return (record, updated, callback) => { 
      if (
        ...
        createAndSendTx(wallet, universal, '0x8de37a84', padLeft(record, 32) + padLeft(updated, 32), callback)
      else 
        callback(400, Messages.INVALID_INPUT)
    }
  },
  ...
  versionRecordSigned: (wallet, universal) => {
    return (record, updated, owner, signature, callback) => { 
      ...
      ].map((val) => { return padLeft(val, 32) }).join('') + padRight(signature, 96) 
      if (
        utils.bytesVerify(record, 32) &&
        utils.bytesVerify(updated, 32) &&
        utils.bytesVerify(owner, 20) &&
...
function getGasPrice(wallet, callback) {
  wallet.provider.eth.getGasPrice().then((price) => { callback(null, price) })
