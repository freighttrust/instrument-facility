...
router.get('/proxy/getTarget', (req, res) => {
  ...
  if (utils.bytesVerify(req.query.id, 4)) {
    req.app.universal_contract.at(req.app.universal_address).then(function (inst) {
...
router.post('/proxy/setTarget', (req, res) => {
  utils.authVerify(req.app, req.body.auth.sig, (auth_error, invalid_message) => {
    if (auth_error) {
      ...
      req.app.setTarget(req.query.id, req.query.target, (res_code, res_msg) => {
...
router.post('/proxy/changeController', (req, res) => {
  utils.authVerify(req.app, req.body.auth.sig, (auth_error, invalid_message) => {
    if (auth_error) {
      ...
      req.app.changeController(req.query.newController, (res_code, res_msg) => {
        res.status(res_code).send(res_msg)
      })
    }
  })
