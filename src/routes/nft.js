...
router.get('/nft/name', (req, res) => {
  req.app.nft_contract.at(req.app.nft_address).then(function (inst) {
...
router.get('/nft/tokenOfOwnerByIndex', (req, res) => {
  ...
  if (utils.bytesVerify(req.query.owner, 20) && utils.bytesVerify(req.query.index, 32)) {
    req.app.nft_contract.at(req.app.nft_address).then(function (inst) {
...
router.get('/nft/isApprovedForAll', (req, res) => {
  ...
  if (utils.bytesVerify(req.query.owner, 20) && utils.bytesVerify(req.query.operator, 20)) {
    req.app.nft_contract.at(req.app.nft_address).then(function (inst) {
...
router.post('/nft/safeTransferFrom', (req, res) => {
  utils.authVerify(req.app, req.body.auth.sig, (error, invalid_message) => {
    if (error) {
      ...
      req.app.safeTransferFrom(req.query.from, req.query.to, req.query.tokenId, req.query.extraData, (res_code, res_msg) => {
...
router.post('/nft/transferFrom', (req, res) => {
  utils.authVerify(req.app, req.body.auth.sig, (error, invalid_message) => {
    if (error) {
      ...
      req.app.transferFrom(req.query.from, req.query.to, req.query.tokenId, (res_code, res_msg) => {
        res.status(res_code).send(res_msg)
      })
    }
  })
...
router.post('/nft/approve', (req, res) => {
  utils.authVerify(req.app, req.body.auth.sig, (error, invalid_message) => {
    if (error) {
      ...
      req.app.approve(req.query.approved, req.query.tokenId, (res_code, res_msg) => {
        res.status(res_code).send(res_msg)
      })
    }
  })
...
router.post('/nft/setApprovalForAll', (req, res) => {
  utils.authVerify(req.app, req.body.auth.sig, (error, invalid_message) => {
    if (error) {
      ...
      req.app.setApprovalForAll(req.query.operator, req.query.status, (res_code, res_msg) => {
        res.status(res_code).send(res_msg)
      })
    }
  })
...
router.post('/nft/signedTransfer', (req, res) => {
  utils.authVerify(req.app, req.body.auth.blockArrayAuth.sig, (error, invalid_message) => {
    if (error) {
      res.status(425).send(error) 
    } else if (invalid_message) {
      res.status(400).send(invalid_message)
    } else {
      req.app.signedTransfer(req.query.from, req.query.to, req.query.tokenId, req.body.auth.senderAuth.sig, (res_code, res_msg) => {
        res.status(res_code).send(res_msg)
      })
    }
  })
...
router.post('/nft/createRecord', (req, res) => {
  utils.authVerify(req.app, req.body.auth.blockArrayAuth.sig, (error, invalid_message) => {
    if (error) {
      ...
      req.app.createRecord(req.query.recordId, req.query.owner, req.query.participant, req.body.auth.ownerAuth.sig, req.body.auth.partAuth.sig, (res_code, res_msg) => {
        res.status(res_code).send(res_msg)
      })
    }
  })
...
router.post('/nft/versionRecord', (req, res) => {
  utils.authVerify(req.app, req.body.auth.sig, (error, invalid_message) => {
    if (error) {
      ...
      req.app.versionRecord(req.query.currentHash, req.query.newHash, (res_code, res_msg) => {
...
router.post('/nft/versionRecord/signed', (req, res) => {
  utils.authVerify(req.app, req.body.auth.blockArrayAuth.sig, (error, invalid_message) => {
    if (error) {
      ...
      req.app.versionRecordSigned(req.query.currentHash, req.query.newHash, req.query.owner, req.body.auth.ownerAuth.sig, (res_code, res_msg) => {
        res.status(res_code).send(res_msg)
      })
    }
  })
