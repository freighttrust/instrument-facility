...
app.universal_contract = truffle_contract(require(getPath(3) + 'ethereum/build/contracts/UniversalABI.json'))
...
app.controller_contract = truffle_contract(require(getPath(3) + 'ethereum/build/contracts/Controller.json'))
...
async.series([
  ...
  (callback) => {
    ...
    setTimeout(proxy_runner, 20000, 'change_master', error_callback(callback))
  },
  (callback) => {
  ...
  (callback) => {
    ...
    setTimeout(proxy_runner, 20000, 'change_controller', error_callback(callback))
  }, 
  (callback) => {
    console.log('Check Controller Test in progress')
    setTimeout(proxy_runner, 20000, 'check_controller', error_callback(callback))
  }, 
  (callback) => {
  ...
  /******************* NFT Tests *********************/
  (callback) => {
    setTimeout(nft_runner, 20000, 'create_record_post', error_callback(callback))
  },
  (callback) => {
    setTimeout(nft_runner, 20000, 'create_record_getters', error_callback(callback))
  },
  (callback) => {
    setTimeout(nft_runner, 20000, 'owner_transfer_from', error_callback(callback))
  },
  (callback) => {
    setTimeout(nft_runner, 20000, 'owner_transfer_getters', error_callback(callback))
  }
], (error) => {
