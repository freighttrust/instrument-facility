...
const owner = new ethers.Wallet('0x290decd9548b62a8d60345a988386fc84ba6bc95484008f6362f93160ef3e563', ganache_web3)
...
function setContractABI(_app, _contract, _abi_filename) {
  _app[_contract] = truffle_contract(require(getPath() + 'ethereum/build/contracts/' + _abi_filename)) 
  ...
  if (typeof _app[_contract].currentProvider.sendAsync !== 'function') {
      ...
      _app[_contract].currentProvider, arguments
    ) 
  }
}
...
function setPostFunctions(_app, _wallet, _controller, _universal) {
  ...
  _app.setApprovalForAll = contract_utils.setApprovalForAll(_wallet, _universal)
  ...
  _app.versionRecordSigned = contract_utils.versionRecordSigned(_wallet, _universal)
}
...
contract('API Testing', (accounts) => {
  ...
  let newman_string = getPath() + 'node_modules/.bin/newman run ' + getPath() + 'tests/postmanTest/BlockArray.postman_collection.json --folder '
  ...
  before(async () => {
    ...
    controller = await Controller.new(universal.address, owner.address).should.be.fulfilled
    app.controller_address = controller.address
    // Change the controller of the universal proxy to be the controller address
    await universal.changeController(controller.address, { from: admin }).should.be.fulfilled
  ...
  it('Postman NFT Tests', (done) => { 
    exec(newman_string + 'nft -d ' + getPath() + 'tests/postmanTest/nftTests.csv', (stderr, stdout) => {
  ...
  it('Postman Proxy Tests', (done) => {
    exec(newman_string + 'proxy -d ' + getPath() + 'tests/postmanTest/proxyTests.csv', (stderr, stdout) => {
